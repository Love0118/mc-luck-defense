...
- 20,000회 중 **167회 클리어 · 0.835%**.
- Wilson 95% 신뢰구간: **0.718%–0.971%**.
- 시드: 3100000–3119999. 조정용 시드 100000–104999와 분리했습니다.
- 태초 2개 미만 클리어: **0회**.
- 평균 도달 라운드 90.27, 평균 소환 2270.6회.
- 정확히 태초 2개 + 신화 4개 이상을 획득한 시도 203회 중 7회 성공 (3.45%).
...
- 별도 시드 3500000부터 10,000회 중 **0회 클리어**.
- Wilson 95% 신뢰구간: 0.000%–0.038%.
...
61라운드부터 적 체력에 1 + (20 − 1) × (max(0, (라운드 − 60)/40))²를 추가 적용합니다. 1~60라운드 체력은 이전과 같고 100라운드에서는 기존 체력의 20배입니다. 보스에도 동일하게 적용합니다.

자동 배치만 사용한 비교 정책은 5,000회 중 24회 성공 (0.48%). 판매 기준과 소환 규칙은 같고 재배치만 하지 않습니다. 배치 전략이 다른 실제 플레이어의 성공률은 달라집니다.
