...
public final class AchievementSmokePlugin extends JavaPlugin implements Listener {
    ...
    private void verify(Player player) throws Exception {
        ...
        if(Files.exists(Path.of("achievement-first-pass.json"))) {
            ...
            Files.writeString(Path.of("achievement-restart-pass.json"),"{\"persistentStats\":true,\"persistent50Awards\":true,\"noDuplicateCompletions\":true}");
        } else {
            ...
            require(completed-start==50,"Exactly fifty completions, got "+(completed-start));
            reached.invoke(service,player,10);reached.invoke(service,player,2000);
            require(completed-start==50,"Round revisits do not repeat rewards");
            ...
            Files.writeString(Path.of("achievement-first-pass.json"),"{\"nativeAdvancements\":50,\"exactlyOnce\":true,\"vanillaBlocked\":true,\"challengeFrames\":true}");
        }
