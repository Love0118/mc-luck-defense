...
SCENARIOS = ("none", "equipped", "top_growth", "top_combat", "top_fusion")
ROUNDS = (30, 60, 100, 200, 300, 400, 500, 600, 700, 1000, 1500, 2000,
          2500, 3000, 4000, 5000, 6000, 7500, 9000, 10000)
NAMES = dict(none="무특성", equipped="중간 티어 장착", top_growth="최고 성장", top_combat="피해·공속 비교", top_fusion="최고 합성·전투")
...
def analyze(folder, expected_runs, output):
    ...
    require(int(rules["roundCap"]) == 10000 and int(rules["batchTicks"]) == 500, "Wrong simulation limits")
    ...
    for summary in summaries:
        ...
        with gzip.open(source, "rt", encoding="utf-8") as stream:
            for i, line in enumerate(stream):
                ...
                require(1 <= r <= 10000 and c in (r-1, r), f"{name}: invalid round")
                ...
                require(not capped or (r == c == 10000 and t == 6000300), "Incomplete capped game")
                checkpoints = row["checkpoints"]
                cr = [x["round"] for x in checkpoints]
        ...
        reached = {r:sum(v for k,v in hist.items() if k >= r) for r in ROUNDS}
    ...
    analysis=dict(validated_games=expected_runs*len(names),round_cap=10000,batch_ticks=500,scenarios=results)
...
def report_markdown(analysis):
    ...
    lines += ["", "| 구성 | 1,000→2,000 | 2,000→2,500 | 2,500→5,000 | 5,000→10,000 |", "|---|---:|---:|---:|---:|"]
    for name,case in cases.items():
        v=case['reached']
        lines.append(f"| {NAMES[name]} | "+" | ".join(f"{v[b]}/{v[a]} ({v[b]/v[a]*100:.2f}%)" if v[a] else "관측 없음" for a,b in [(1000,2000),(2000,2500),(2500,5000),(5000,10000)])+" |")
    lines += ["", "같은 시드를 각 구성에 적용했다. 시드 누락·중복·순서, 판 수, 라운드·처리 틱, 보상 값과 실행기 요약을 원본 전체에서 다시 검증했다.",
              ...
              "", "이는 고정된 자동 구매·판매·배치·합성 정책의 결과다. 실제 이용자 전체의 통계나 가능한 모든 특성 조합의 최적해가 아니다. 10,000라운드는 시뮬레이션 관측 상한이며 강제 승리 조건이 아니다."]
    return "\n".join(lines)+"\n"
