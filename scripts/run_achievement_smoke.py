"""Two normal Paper boots verify native advancement persistence; localhost only."""
    ...
    for attempt in [1,2]:
        with (run/f"server-{attempt}.log").open("w",encoding="utf-8") as log:
            ...
            try:
                ...
                required=run/("achievement-first-pass.json" if attempt==1 else "achievement-restart-pass.json")
