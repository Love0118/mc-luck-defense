...
'신화·태초 소환 및 진 태초 승급은 서버 전체 채팅 방송과 접속자 전원에게 소리를 전달합니다. 진 태초는 뽑기 횟수가 아닌 실제 승급 횟수를 따로 누적합니다. 진 태초 승급 시 원래 뽑힌 등급의 방송은 중복해서 보내지 않습니다. 전설·에픽은 소환자의 기존 피드백만 유지합니다. 도전과제 최초 달성과 승급이 동시에 발생하면 각각의 알림이 발생할 수 있습니다.','',
'| 번호 | 이름 | 조건 | 알림 |','|---:|---|---|---|']
index=0
for metric,thresholds,names,hard in re.findall(r'add\(entries, Metric\.(\w+), new long\[\]\{([^}]+)\},\s*new String\[\]\{([^}]+)\},(\d+)\)',source):
    for target,title in zip(map(int,thresholds.split(',')),re.findall(r'"([^"]+)"',names)):
        index+=1
        label={'EPIC':'에픽','MYTHIC':'신화','PRIMORDIAL':'태초','TRUE_PRIMORDIAL':'진 태초'}.get(metric)
        action='승급' if metric=='TRUE_PRIMORDIAL' else '소환'
        condition=f'{target:,}라운드 도달' if metric=='ROUND' else f'{label} 누적 {target:,}회 {action}'
        text.append(f'| {index} | {title} | {condition} | {"챌린지 + 완료음" if target>=int(hard) else "일반 토스트"} |')
assert index==60,index
