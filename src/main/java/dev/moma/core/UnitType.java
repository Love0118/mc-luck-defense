...
public enum UnitType {
    ...
    BOGGED("보그드", RANGED_SINGLE, 11.5, 18, 8, 0, 1),
    ...
    ALLAY("알레이", MULTI_TARGET, 4.8, 16, 7, 0, 3),
    ...
    UnitType(String label, AttackRole role, double damage, int interval, double range, double radius, int targets) {
        ...
        double compensation = switch (role) {
            case MELEE_SINGLE -> 2.8;
            case MULTI_TARGET -> 1.8;
            default -> 1.0;
        };
        this.profile = new CombatProfile(damage * compensation, interval, range, radius, targets);
    }
