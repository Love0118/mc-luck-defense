...
public record CampaignRules(int gridSize, long startingCoins, int enemyLimit, int preparationTicks,
                            int roundTicks, int cleanupTicks, double healthScale) {
    ...
    public static CampaignRules standard() {
        ...
        return new CampaignRules(Integer.parseInt(p.getProperty("grid-size")), Long.parseLong(p.getProperty("starting-coins")),
                ...
                Double.parseDouble(p.getProperty("health-scale")));
    }
    public CampaignRules withHealthScale(double scale) { return new CampaignRules(gridSize, startingCoins, enemyLimit, preparationTicks, roundTicks, cleanupTicks, scale); }
