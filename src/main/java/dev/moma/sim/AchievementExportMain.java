...
public final class AchievementExportMain {
    public static void main(String[] args) {
        ...
        System.out.println("\n챌린지 업적마다 보상 하나를 해금합니다. 총 "+TraitCatalog.ALL.size()+"개 보상이며, 장착 특성은 최대 4개입니다. 네 번째 슬롯은 시즌 1에서 1500라운드 도달로 해금합니다. 패시브는 슬롯을 사용하지 않습니다.");
        ...
        for(var achievement:AchievementCatalog.ALL) {
            ...
            System.out.println("| "+achievement.id()+" | "+achievement.title()+" | "+achievement.description()+" | "+
                    (trait==null?"없음":trait.name()+(trait.passive()?" (패시브)":""))+" | "+(trait==null?"일반 토스트":trait.description())+" |");
        }
