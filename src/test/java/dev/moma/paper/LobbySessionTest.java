...
class LobbySessionTest {
    ...
    @Test void simultaneousStartsAreExclusiveAndFreedSlotsCreateFreshSessions() {
        ...
        games.start(first,true); GameSession previous=games.session(first);
        assertTrue(previous.arena.smallForce());assertEquals(10,previous.arena.deploymentLimit());
        ...
        games.start(second);
        assertFalse(games.session(second).arena.smallForce());assertEquals(36,games.session(second).arena.deploymentLimit());
        ...
        games.start(third);
        assertFalse(games.session(third).arena.smallForce());
